   ...
   # if reservations['ResponseMetadata']['HTTPStatusCode'] == 200:
    ...
    for reservation in reservations:
        for instance in reservation["Instances"]:
             ...
             Parameters={
               "commands": ["runuser -l  applmgr -c 'cd auto_switch; sh atsappctl -target_inst "+tgt_db+" -primary_db "+prm_db+" -action STOP_APPS'"]
        ...
        if response['ResponseMetadata']['HTTPStatusCode'] != 200:
           raise Exception ("Issue in Stopping App Services")
    ...
    statusCode=201
    print('***Error - Failed to run ssm send command to stop application services.')
    print(type(e), ':', e)
    return {
              'StatusCode': statusCode,
              'CommandId' : "NULL",
